...
def func_3():
    ...
    try:
            ...
            for fund in funds_to_show:
                try:
                    ...
                    if len(one_year_data) > 0:
                        ...
                        max_nav_year = one_year_data.loc[max_idx, '单位净值']
                        max_date_year = one_year_data.loc[max_idx, '净值日期'].strftime('%m-%d')
                        # 计算从过去一年最高点到当前净值的跌幅
                        drawdown_from_peak = (max_nav_year - latest_nav) / max_nav_year * 100
                    else:
                        max_nav_year = None
                        max_date_year = None
                    ...
                    table_data.append({
                        ...
                        "peak_date": max_date_year,
                        "peak_nav": max_nav_year
                    })
        ...
        for item in table_data:
            ...
            # 距一年高点跌幅格式化（包含高点日期）
            if item["drawdown_from_peak"] is None:
                dd_str = "--"
            elif item["drawdown_from_peak"] > 0:
                dd_str = f'<span style="color: green; font-weight: bold;">-{item["drawdown_from_peak"]:.2f}%</span> <span style="color: gray;">({item["peak_date"]})</span>'
            elif item["drawdown_from_peak"] < 0:
                dd_str = f'<span style="color: red; font-weight: bold;">+{abs(item["drawdown_from_peak"]):.2f}%</span> <span style="color: gray;">({item["peak_date"]})</span>'
            else:
                dd_str = f'<span style="color: gray;">0.00%</span> <span style="color: gray;">({item["peak_date"]})</span>'
